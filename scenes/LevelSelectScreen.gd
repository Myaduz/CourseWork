...
func create_level_btn(lvl_path, lvl_name):
	...
	var formatted_name = lvl_name
	if '.tscn.remap' in formatted_name:
				formatted_name = formatted_name.trim_suffix('.remap')
	btn.text = formatted_name.trim_suffix('.tscn').replace("_", " ")
